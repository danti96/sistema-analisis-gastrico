                    ...
                    <x-slot name="tbody">
                        <template x-for="(item, index) in table.data" :key="index">
                            <tr class="hover:bg-gray-100 border-b">
                                <x-alpinejs.table.td>
                                    <a x-bind:href="`${ ('{{ route('paciente.show', ['paciente' => ':paciente']) }}').replace(':paciente', item.id)}`"
                                    ...
                                    <x-alpinejs.table.options.dropdown>
                                        <x-slot name="slotDropdown">
                                            <x-alpinejs.table.options.option
                                                x-bind:href="`${ ('{{ route('paciente.edit', ['paciente' => ':paciente']) }}').replace(':paciente', item.id)}`">
                                            ...
                                            <x-alpinejs.table.options.option
                                                x-bind:href="`${ ('{{ route('paciente.show', ['paciente' => ':paciente']) }}').replace(':paciente', item.id)}`">
                                                <svg class="w-6 h-6 text-gray-800" aria-hidden="true"
                                                    xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                    fill="none" viewBox="0 0 24 24">
                                                    <path stroke="currentColor" stroke-linecap="round"
